...
async def test_fetch_models_success(mock_httpx_client, mock_endpoint, sample_models):
    ...
    with patch("comfyui_ollama_model_manager.ollama_client.httpx.AsyncClient") as mock_client_cls:
...
async def test_load_model_success(mock_httpx_client, mock_endpoint):
    ...
    with patch("comfyui_ollama_model_manager.ollama_client.httpx.AsyncClient") as mock_client_cls:
        ...
        assert result == {"models": [{"name": name} for name in ["llama3.2", "mistral", "phi3"]]}
